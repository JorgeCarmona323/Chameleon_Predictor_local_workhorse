...
def load_registry():
    """REFERENCE_COMPOUNDS from the OPTIONAL scripts/crest_v3.2.py registry. Not part of the
    core install; --compound needs it, --smiles does not."""
    reg = SCRIPTS / "crest_v3.2.py"
    if not reg.exists():
        sys.exit('error: --compound needs the optional reference registry '
                 '(scripts/crest_v3.2.py), which is not in this core install. '
                 'Use --smiles "<SMILES>" --name <name> instead.')
    spec = importlib.util.spec_from_file_location("crest_v32", reg)
