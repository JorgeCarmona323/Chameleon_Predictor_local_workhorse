...
![AUC by subset](../../results/figures/auc_by_subset.png)

**Figure 0. Single-feature AUC-ROC of key descriptors on the clean subset (Furukawa + Chugai,
blue) vs. the full dataset (orange); threshold −6.0 log cm/s.** The two 3D ensemble
descriptors are shaded. Absolute ΔPSA and per-MW-normalized ΔPSA both carry signal on the
clean subset (≈0.69 / 0.59) that collapses on the full dataset (0.51 / 0.47), while the DB
single-structure ΔPSA stays at chance in both. MolLogP *inverts* on the clean subset
(AUC 0.32, i.e. 0.68 flipped) — the large polar macrolides dominate the permeable population,
reversing the usual lipophilicity/permeability relationship. *Generated by
`scripts/ml/auc_by_subset.py` (y-axis from 0, so no bars are clipped).*
...
> **Cluster-validity metrics (read this once).** K-Medoids is scored with **silhouette**
> (built for partitional, roughly convex clusters); HDBSCAN with **DBCV** (built for
> density-based clusters that have noise and arbitrary shapes). Earlier Panel A/B images
> mislabeled HDBSCAN with a silhouette value — they are being regenerated so each algorithm
> reports its correct metric, consistent with Panel C. Cluster *counts* below are unaffected.

### Panel A — 2D descriptors (n = 6,938)

![Panel A 2D UMAP](../../results/figures/Panel_A_2D_umap.png)

**Figure 1. UMAP of the 2D topological feature set (6,938 compounds).** Track A: K-Medoids
k=8 archetypes, **silhouette 0.438**, cluster permeability 36–90%. Track B: HDBSCAN shatters
into **69 clusters + 914 noise points** (poor DBCV) — no coherent density structure. Clincher:
...
**Figure 2. UMAP of the 3D Δ feature set on all 6,938 compounds.** Track A: K-Medoids k=8,
**silhouette 0.241**. Track B: HDBSCAN collapses to **2 natural clusters** (C0 n=570,
C1 n=5,930) + 437 noise — the low-dimensional two-population structure the 2D set could not
find. Clincher: color-mixed — the permeability signal is washed out by pooled multi-source
label noise (K-Medoids cluster permeability a flat 64–71%, no enrichment). *3D Δ finds real
structure even where the labels are too noisy to track it.*

### Panel B — 3D Δ descriptors, clean subset (n = 1,502)

![Panel B 3D delta UMAP, 1502](../../results/figures/Panel_B_3D_delta_umap_1502.png)

**Figure 3. Same 3D Δ embedding on the Furukawa + Chugai subset (~1,502 compounds).** Track A:
K-Medoids k=8, **silhouette 0.235**. Track B: the same **2 HDBSCAN clusters** (C0 n=695,
73% permeable; C1 n=481, 25% permeable), now cleanly separated. Clincher: a clear spatial
gradient — **left lobe impermeable-enriched (red)**, **right lobe permeable-enriched (green)**,
with CsA (★) in the green lobe. *On clean labels, the same structure that survived at scale
also tracks permeability — with no knowledge of the PAMPA values.*

> **2D vs 3D:** 69 fragmented clusters (2D) vs. 2 clean clusters (3D). The 3D Δ descriptors
> compress this space into a genuine low-dimensional, two-population structure that 2D
> descriptors cannot find.
...
**Figure 4. Combined 2D+3D embedding, Furukawa + Chugai (1,566 compounds).**
Track A: K-Medoids k=8 (**silhouette 0.256**) — 8 structural families, K3 at 98% permeable.
Track B: HDBSCAN 2 clusters (C0 n=883, 88% permeable; C1 n=651, 59%; **DBCV 0.004**) — this run
is **extremely stable (min pairwise ARI 0.995 across 5 seeds)**, one dominant structure.
(On regeneration this panel also carries **Track E**, residue count — see below.)
