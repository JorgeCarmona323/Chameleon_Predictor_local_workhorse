...
- remove hour per compound - Ricardo
...
include a way we can return to the run to add conformers ontop of the 200. develop a save state function, look into documentation so we can add conformers since we are starting with only 200 so we can add more conformers if ti works
...
if crest can handle sdf we go with that
...
-update to run in parallel cpu update to up to 20
